# Problem 1
# Create a function called mult that has two parameters, the first is required and should be an integer, 
# the second is an optional parameter that can either be a number or a string but whose default is 6. 
# The function should return the first parameter multiplied by the second.

def mult(a,b = 6):
    return int(a)*int(b)

print("product is :",mult(7))

# Problem 2
# Write a function greeting that has three parameters greeting, name, excl.
# name is required while the optional parameter greeting has the default value of "Hello " 
# and the optional parameter excl has the default value of "!". 
# The function returns the string greeting name excl.

def greeting(name,greeting='Hello',excl = '!'):
    return greeting + " " + name + excl

print(greeting("bob"))

# Problem 3
# Write a function sum that has two parameters intx, intz.
# intx is required while optional parameter intz has the default value of 5. 
# sum returns the sum of the two parameters.

def sum(intx,intz = 5):
    return intx + intz


# Problem 4
# Write a function, test, that takes in three parameters: a required integer, an optional boolean whose default value is True,
# and an optional dictionary, called dict1, whose default value is {2:3, 4:5, 6:8}. If the boolean parameter is True, 
# the function should test to see if the integer is a key in the dictionary. The value of that key should then be returned. 
# If the boolean parameter is False, return the boolean value False.

def test(num,boolean = True,dict1 ={2:3,4:5,6:6}):
        if boolean:
            if num in dict1:
                return dict1[num]
        return False     
 

dict2 ={3:6,9:0,7:5}         
print(test(3,True,dict2))

# Problem 5
# Write a function called checkingIfIn that takes three parameters. The first is a required parameter, which should be a string.
# The second is an optional parameter called direction with a default value of True. 
# The third is an optional parameter called d that has a default value of:
# {'apple': 2, 'pear': 1, 'fruit': 19, 'orange': 5,'banana': 3, 'grapes': 2, 'watermelon': 7}.
# When the second parameter is True, it should check to see if the first parameter is a key in the third parameter; 
# if it is, return True, otherwise return False.
# But if the second paramter is False, then the function should check to see if the first parameter is not a key of the third. 
# If it’s not, the function should return True in this case, and if it is, it should return False.

initial = {'apple': 2, 'pear': 1, 'fruit': 19, 'orange': 5,'banana': 3, 'grapes': 2, 'watermelon': 7}
def checkingIfIn(astring,direction = True, d = initial):
     if direction == True :
          if str(astring) in d.keys():
               return True
          else:
               return False
     if direction == False :
          if str(astring) not in d.keys():
               return True
          else:
               return False
          

print(checkingIfIn("red",True))          
                    
     
     